...
from vouchcode import AUTHOR_NAME, AUTHOR_URL, PROJECT_NAME, REPOSITORY_URL, __version__
...
def render_pdf(report: dict[str, Any], destination: Path) -> Path:
    ...
    document = SimpleDocTemplate(
        ...
        author=AUTHOR_NAME,
        subject="AI authorship provenance and comprehension verification",
        creator=f"{PROJECT_NAME} {__version__} ({REPOSITORY_URL})",
    )
...
def _colophon(styles: dict[str, ParagraphStyle]) -> list[Any]:
    """Name the tool that produced this document, and who wrote it.

    A report handed to a supervisor or a hiring manager should say what generated it and
    where that came from, so a reader who wants to check the method has somewhere to go
    rather than only the numbers in front of them.
    """
    return [
        Spacer(1, 8 * mm),
        Paragraph(
            f"Generated by {_escape(PROJECT_NAME)} {_escape(__version__)}, "
            f"{_escape(REPOSITORY_URL)}",
            styles["meta"],
        ),
        Paragraph(
            f"Author: {_escape(AUTHOR_NAME)}, {_escape(AUTHOR_URL)}",
            styles["meta"],
        ),
    ]
